...
/*
 * This function will be called by the df command.	/////////////////// hint!!! [YC]
 */
...
ux_statfs(struct super_block *sb, struct statfs *buf)
{
        ...
        buf->f_blocks = usb->s_nrblocks;		// [YC]
...
ux_read_super(struct super_block *s, void *data, int silent)
{
	...
	sb_size = sizeof(struct ux_superblock);		// [YC]
	if ((sb_size + 1) % UX_BSIZE)			// [YC]
		sb_nr = sb_size / UX_BSIZE + 1;		// [YC]
	s->s_blocksize = UX_BSIZE * sb_nr;		// [YC]
	
        s->s_blocksize_bits = UX_BSIZE_BITS;
	
        bh = sb_bread(s, 0);				// Must maintain ordering of these instructions.
        if(!bh) {
                goto out;
        }
	...
	s->s_blocksize = UX_BSIZE;			// [YC] Eureka! ON: issue#4 solved.
        /*
