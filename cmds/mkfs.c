...
main(int argc, char **argv)
{
        ...
        int                     map_blks;
        char                    block[UX_BSIZE];
	unsigned long		f_size;					// [YC] protected from overflow.
        ...
        f_size = lseek(devfd, 0, SEEK_END);				// [YC] where is the end?
	if (error == -1) {
	...
	sb.s_nrblocks = f_size / UX_BSIZE;				// [YC] Fill in the new field.
        sb.s_magic = UX_MAGIC;
        sb.s_mod = UX_FSCLEAN;
        sb.s_nifree = UX_MAXFILES - 4; 					// [YC] number of inodes free: 28.
        sb.s_nbfree = sb.s_nrblocks - 2;				// [YC] number of blocks free.
